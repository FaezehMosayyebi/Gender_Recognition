...
class Trainer():
    ...
    def load_data(self):
        try:
            ...
            self.validation_dataset = image_dataset_from_directory(self.valid_dir,
                                                    label_mode="categorical",
                                                    shuffle=True,
                                                    batch_size=self.batch_size,
                                                    image_size=self.img_size)
    ...
    def data_augmentation(self, rotation_factor, translation_factor, flip, contrast_factor):
        ...
        if rotation_factor is not None:
            data_augmentation.add(tf.keras.layers.experimental.preprocessing.RandomRotation(factor=rotation_factor))

        if translation_factor is not None:
            data_augmentation.add(tf.keras.layers.experimental.preprocessing.RandomTranslation(height_factor=translation_factor[0], width_factor=translation_factor[1]))

        if flip is not None:
            data_augmentation.add(tf.keras.layers.experimental.preprocessing.RandomFlip(mode=flip))

        if contrast_factor is not None:  
            data_augmentation.add(tf.keras.layers.experimental.preprocessing.RandomContrast(factor=contrast_factor))
    ...
    def train(self, num_epochs, learning_rate):
        ...
        self.train_history = self.model.fit(self.train_dataset,
    ...
    def fine_tune(self, tune_from, num_epochs):
        ...
        self.model.compile(loss=tf.keras.losses.CategoricalCrossentropy(),
              optimizer = tf.keras.optimizers.RMSprop(learning_rate=self.learning_rate/10),
              metrics=['accuracy'])
    ...
    def plot_whole_process(self, save_to_dir):
        ...
        val_loss = self.train_history.history['val_loss'] + self.history_fine.history['val_loss']
        ...
        plt.plot(val_acc, label='Validation Accuracy')
        plt.ylim([0.8, 1])
        plt.plot([self.train_num_epochs-1,self.train_num_epochs-1],
